...
from .train_pytorch_nfeat import train_with_nfeat, train_with_nfeat_mp
...
from .models.ensemble_train_models_with_nfeat import EnsembleKEModelWithNFeat


def load_model(args, n_entities, n_relations, ent_feat_dim, other_ent_feat_dim, rel_feat_dim, ckpt=None):
    model = EnsembleKEModelWithNFeat(args, "concat", args.model_name, n_entities, n_relations,
...
def main():
    ...
    from .dataloader.ensemble_dataset import KGDatasetWikiEnsembleNFeat
    dataset = KGDatasetWikiEnsembleNFeat(args)
    ...
    model = load_model(args, dataset.n_entities, dataset.n_relations, dataset.entity_feat.shape[1],
                       dataset.other_nfeat_train.shape[1], dataset.relation_feat.shape[1])
    if args.encoder_model_name in ['roberta', 'concat']:
        model.entity_feat.emb = dataset.entity_feat
        model.other_entity_feat['train'].emb = dataset.other_nfeat_train
        model.other_entity_feat['valid'].emb = dataset.other_nfeat_valid
        model.other_entity_feat['test'].emb = dataset.other_nfeat_test
        print(dataset.other_nfeat_train.shape, dataset.other_nfeat_valid.shape, dataset.other_nfeat_test.shape)
    ...
    if args.num_proc > 1:
        ...
        for i in range(args.num_proc):
            ...
            proc = mp.Process(target=train_with_nfeat_mp, args=(args,
                                                     model,
                                                     train_samplers[i],
                                                     valid_sampler,
                                                     test_sampler,
        ...
        train_with_nfeat(args, model, train_sampler, valid_samplers, test_samplers, rel_parts=rel_parts)
