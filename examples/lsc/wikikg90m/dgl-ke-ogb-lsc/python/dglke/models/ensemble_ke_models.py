...
class EnsembleKEModel(object):
    ...
    def __init__(self, args, encoder_model_name, model_name, n_entities, n_relations, hidden_dim, gamma,
                 ...
                 , rel_feat_dim=-1):
        super(EnsembleKEModel, self).__init__()
        self.args = args
        ...
        if self.encoder_model_name == 'concat':
            ...
            self.transform_net = MLP(ent_feat_dim, entity_dim, rel_feat_dim, relation_dim, args.mlp_version)
        ...
        if model_name == 'TransE' or model_name == 'TransE_l2':
            ...
            self.score_func = PairREScore(gamma, self.emb_init)
            ...
            if self.encoder_model_name == 'roberta':
                ...
                neg_head = self.transform_net.embed_entity(torch.cat(
                    [self.entity_feat(neg_head_ids, gpu_id, False),
                     self.other_entity_feat(neg_head_ids, gpu_id, False),
                     self.entity_emb(neg_head_ids, gpu_id, trace)], -1))
            ...
            if self.encoder_model_name == 'roberta':
                ...
                neg_tail = self.transform_net.embed_entity(torch.cat(
                    [self.entity_feat(neg_tail_ids, gpu_id, False),
                     self.other_entity_feat(neg_tail_ids, gpu_id, False),
                     self.entity_emb(neg_tail_ids, gpu_id, trace)], -1))
    ...
    def forward_test_wikikg(self, query, ans, candidate, mode, gpu_id=-1, topk=10):
        ...
        return argsort[:, :topk], F.topk(scores, k=topk, dim=1, descending=True)
    ...
    def predict_score_wikikg(self, query, candidate, mode, to_device=None, gpu_id=-1, trace=False):
        ...
        if mode == 't,r->h':
            if self.encoder_model_name == 'roberta':
                ...
                tail = self.transform_net.embed_entity(torch.cat(
    ...
    def forward(self, pos_g, neg_g, gpu_id=-1):
        ...
        if self.encoder_model_name == 'roberta':
            ...
            pos_g.ndata['emb'] = self.transform_net.embed_entity(torch.cat(
                [self.entity_feat(pos_g.ndata['id'], gpu_id, False),
                 self.other_entity_feat(pos_g.ndata['id'], gpu_id, False),
                 self.entity_emb(pos_g.ndata['id'], gpu_id, True)],
                -1))
            pos_g.edata['emb'] = self.transform_net.embed_relation(torch.cat(
                [self.relation_feat(pos_g.edata['id'], gpu_id, False),
                 self.relation_emb(pos_g.edata['id'], gpu_id, True)], -1))
